...
G4VPhysicalVolume* RES_DetectorConstruction::Construct()
{
  ...
  for (unsigned int i = 0; i < m_modulePlacements.size(); i++) {
    m_module_phys.push_back(new G4PVPlacement(0, m_modulePlacements[i], m_module_log, "module", m_world_log, false, i));
  }

  // interior of modules
  m_module_upper_layer_solid = new G4Box("module_upper_layer", 0.5*m_moduleLength, 0.5*m_moduleWidth, 0.5*m_moduleLayerThickness);
  m_module_upper_layer_log = new G4LogicalVolume(m_module_upper_layer_solid, m_world_material, "module_upper_layer", 0, 0, 0);
  m_module_upper_layer_phys = new G4PVPlacement(0, G4ThreeVector(0, 0, 0.5*m_moduleGap + 0.5*m_moduleLayerThickness), m_module_upper_layer_log, "module_upper_layer", m_module_log, false, 0);
  
  G4RotationMatrix* rotation = new G4RotationMatrix(m_moduleStereoAngle, 0., 0.);
  m_module_lower_layer_solid = new G4Box("module_lower_layer", 0.5*m_moduleLength, 0.5*m_moduleWidth, 0.5*m_moduleLayerThickness);
  m_module_lower_layer_log = new G4LogicalVolume(m_module_lower_layer_solid, m_world_material, "module_lower_layer", 0, 0, 0);
  m_module_lower_layer_phys = new G4PVPlacement(rotation, G4ThreeVector(0, 0, -0.5*m_moduleGap - 0.5*m_moduleLayerThickness), m_module_lower_layer_log, "module_lower_layer", m_module_log, false, 0);
  ...
  m_module_upper_layer_log->SetSensitiveDetector(aFiberSD);
  m_module_lower_layer_log->SetSensitiveDetector(aFiberSD);
...
void RES_DetectorConstruction::SetVisibility()
{
  ...
  m_module_upper_layer_log->SetVisAttributes(vis_att);
  m_module_lower_layer_log->SetVisAttributes(vis_att);
