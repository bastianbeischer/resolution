...
void RES_Module::InitializeCommonValues()
{
  ...
  m_carbonFiberThickness = 0.15*mm;
  m_epoxyThickness = 0.39*mm;
  m_hysolThickness = 0.5*mm;
  m_foamThickness = 7.8*mm - 2*m_epoxyThickness;
  ...
  // Epoxy
  G4Material* Epoxy = new G4Material("Epoxy", 1.2*g/cm3, components=3);
  Epoxy->AddElement(H,natoms=44);
  Epoxy->AddElement(C,natoms=15);
  Epoxy->AddElement(O,natoms=7);

  // Epoxy
  G4Material* Hysol = new G4Material("Hysol", 1.36*g/cm3, components=3);
  Hysol->AddElement(H,natoms=44);
  Hysol->AddElement(C,natoms=15);
  Hysol->AddElement(O,natoms=7);

  // Carbon Fiber Layer
  G4Material* CarbonFiber = new G4Material("Carbon Fibre", density=1.65*g/cm3, components = 2);
  ...
  // Rohacell
  G4Material* Rohacell = new G4Material( "rohacell", density=50.*kg/m3, components = 4 );
...
{
  if (m_type == fiber) {
    m_height = 2*(m_foamThickness + 2*m_carbonFiberThickness + m_fiberThickness + 2*m_epoxyThickness) + m_hysolThickness;
    G4double zInModule = 0.5*m_hysolThickness + m_epoxyThickness + m_carbonFiberThickness + m_foamThickness + m_carbonFiberThickness + m_epoxyThickness + 0.5*m_fiberThickness;
...
G4PVPlacement* RES_Module::Construct(G4VPhysicalVolume* mother, G4int copyNumber)
{
  ...
  if (m_type == fiber) {
    ...
    G4VSolid*           epoxySolid       = new G4Box("moduleEpoxy", 0.5*m_length, 0.5*m_width, 0.5*m_epoxyThickness);
    G4VSolid*           hysolSolid       = new G4Box("moduleHysol", 0.5*m_length, 0.5*m_width, 0.5*m_hysolThickness);
    ...
    if (m_subtractHoles) {
      ...
      hole = new G4Box("hole", 0.5*holeSideLength, 0.5*holeSideLength, 0.51*m_hysolThickness);
      G4SubtractionSolid* hysolSubSolid = new G4SubtractionSolid("hysolSubSolid", hysolSolid, hole, 0, G4ThreeVector(holePositions[0], 0, 0));
      for (int i = 1; i < 7; i++)
        hysolSubSolid = new G4SubtractionSolid("hysolSubSolid", hysolSubSolid, hole, 0, G4ThreeVector(holePositions[i], 0, 0));
      hysolSolid = hysolSubSolid;
    ...
    G4LogicalVolume* hysolLogic       = new G4LogicalVolume(hysolSolid, m_hysolMaterial, "moduleHysol", 0, 0, 0);
    G4LogicalVolume* foamLogic        = new G4LogicalVolume(foamSolid, m_foamMaterial, "moduleFoam", 0, 0, 0);

    m_upperFiberPlacement        = new G4PVPlacement(0, G4ThreeVector(0,0,0.5*m_hysolThickness + m_carbonFiberThickness + m_epoxyThickness + m_foamThickness + m_epoxyThickness + m_carbonFiberThickness + 0.5*m_fiberThickness),
                                                     fiberLogic, "moduleFiber", moduleLogic, false, 0);
    m_firstCarbonFiberPlacement  = new G4PVPlacement(0, G4ThreeVector(0,0,0.5*m_hysolThickness + m_carbonFiberThickness + m_epoxyThickness + m_foamThickness + m_epoxyThickness + 0.5*m_carbonFiberThickness),
                                                     carbonFiberLogic, "moduleCarbonFiber", moduleLogic, false, 0);
    m_firstEpoxyPlacement        = new G4PVPlacement(0, G4ThreeVector(0,0,0.5*m_hysolThickness + m_carbonFiberThickness + m_epoxyThickness + m_foamThickness + 0.5*m_epoxyThickness),
                                                     epoxyLogic, "moduleEpoxy", moduleLogic, false, 0);
    m_upperFoamPlacement         = new G4PVPlacement(0, G4ThreeVector(0,0,0.5*m_hysolThickness + m_carbonFiberThickness + m_epoxyThickness + 0.5*m_foamThickness),
                                                     foamLogic, "moduleFoam", moduleLogic, false, 0);
    m_secondEpoxyPlacement       = new G4PVPlacement(0, G4ThreeVector(0,0,0.5*m_hysolThickness + m_carbonFiberThickness + 0.5*m_epoxyThickness),
                                                     epoxyLogic, "moduleEpoxy",moduleLogic, false, 1);
    m_secondCarbonFiberPlacement = new G4PVPlacement(0, G4ThreeVector(0,0,0.5*m_hysolThickness + 0.5*m_carbonFiberThickness),
                                                     carbonFiberLogic, "moduleCarbonFiber", moduleLogic, false, 1);
    m_hysolPlacement             = new G4PVPlacement(0, G4ThreeVector(0,0,0),
                                                     hysolLogic, "moduleHysol",moduleLogic, false, 0);
    m_thirdCarbonFiberPlacement  = new G4PVPlacement(0, G4ThreeVector(0,0,-0.5*m_hysolThickness - 0.5*m_carbonFiberThickness),
                                                     carbonFiberLogic, "moduleCarbonFiber", moduleLogic, false, 2);
    m_thirdEpoxyPlacement        = new G4PVPlacement(0, G4ThreeVector(0,0,-0.5*m_hysolThickness - m_carbonFiberThickness - 0.5*m_epoxyThickness),
                                                     epoxyLogic, "moduleEpoxy", moduleLogic, false, 2);
    m_lowerFoamPlacement         = new G4PVPlacement(0, G4ThreeVector(0,0,-0.5*m_hysolThickness - m_carbonFiberThickness - m_epoxyThickness - 0.5*m_foamThickness),
                                                     foamLogic, "moduleFoam", moduleLogic, false, 1);
    m_fourthEpoxyPlacement       = new G4PVPlacement(0, G4ThreeVector(0,0,-0.5*m_hysolThickness - m_carbonFiberThickness - m_epoxyThickness - m_foamThickness - 0.5*m_epoxyThickness),
                                                     epoxyLogic, "moduleEpoxy", moduleLogic, false, 3);
    m_fourthCarbonFiberPlacement = new G4PVPlacement(0, G4ThreeVector(0,0,-0.5*m_hysolThickness - m_carbonFiberThickness - m_epoxyThickness - m_foamThickness - m_epoxyThickness - 0.5*m_carbonFiberThickness),
                                                     carbonFiberLogic, "moduleCarbonFiber", moduleLogic, false, 3);
    m_lowerFiberPlacement        = new G4PVPlacement(0, G4ThreeVector(0,0,-0.5*m_hysolThickness - m_carbonFiberThickness - m_epoxyThickness - m_foamThickness - m_epoxyThickness - m_carbonFiberThickness - 0.5*m_fiberThickness),
...
void RES_Module::PrintMaterials()
{
  ...
  // fiber mat:
  G4cout << "Mass of a fiber mat: " << m_upperFiberPlacement->GetLogicalVolume()->GetMass() / g << "g" << G4endl;
  G4cout << "Total mass of a module " << m_modulePlacement->GetLogicalVolume()->GetMass() / g << " g" << G4endl;
